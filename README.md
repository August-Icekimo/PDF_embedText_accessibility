...
提供給程式解析建構標籤的 JSON，必須是一個陣列組合，每個物件代表一個段落或元素，必須包含代表絕對位置的視窗座標 `top`, `left`, `width`, `height` 等欄位。
...
> - **座標定位**：請針對每一個元素估算在 1000x1414 畫布上的精準位置，包含 css_top, css_left, width, height。數值請直接寫整數的 px 值。
